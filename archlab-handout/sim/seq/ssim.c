...
static char *format_prev() {
  ...
  sprintf(status_msg, "%c %s %s %s %s", prev_bcond ? 'Y' : 'N', iname(HPACK(prev_icode, prev_ifun)), istring, mstring,
          pstring);
...
static char *format_f() {
  ...
  sprintf(status_msg, "%s %s %s %s %s", iname(HPACK(icode, ifun)), reg_name(ra), reg_name(rb), valcstring, valpstring);
  return status_msg;
}
...
static char *format_d() {
  ...
  sprintf(status_msg, "%s %s %s %s %s %s", valastring, valbstring, reg_name(destE), reg_name(destM), reg_name(srcA),
          reg_name(srcB));
...
int simRunCmd(ClientData clientData, Tcl_Interp *interp, int argc, char *argv[]) {
  ...
  if (argc >= 2 && (sscanf(argv[1], "%lld", &step_limit) != 1 || step_limit < 0)) {
